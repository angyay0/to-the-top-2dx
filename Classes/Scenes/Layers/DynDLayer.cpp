...
void DynDLayer::configureWinner(){
    Size viewPort = Director::getInstance()->getVisibleSize();
    Vec2 origin = Director::getInstance()->getVisibleOrigin();
    float scale = Director::getInstance()->getContentScaleFactor()/2.f;
    
    auto playMenuItem = MenuItemImage::create("next.png","next.png",CC_CALLBACK_1(DynDLayer::onClick, this));
    playMenuItem->setTag(NEXT_TAG);
    playMenuItem->setScale(scale);
    ...
    auto pauseTitle = Label::createWithTTF("Level Completed", "fonts/Marker Felt.ttf", 36);
